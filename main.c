...
void choose_move(int *bot_pos, int *next_move, char *last_move, int *last_move_array) {
    ...
    *last_move = x == 1 ? 'D' : x == -1 ? 'U' : y == 1 ? 'R' : y == -1 ? 'L' : 'N';
    ...
    update_array(last_move_array, x, y);
}
...
int main(){
    ...
    int bot_pos[2] = {0, 0}, last_move_array[2] = {0, 0}, next_move[2] = {0, 0}, cleaned = 1, total_squares = 0;

    for(int i = 0; i < N; i++) // Cuento las casillas a limpiar
        for(int j = 0; j < N; j++)
            area[i][j] == '.' ? total_squares++ : 0;
    area[0][0] = 'C'; // "Limpio" la casilla de salida
